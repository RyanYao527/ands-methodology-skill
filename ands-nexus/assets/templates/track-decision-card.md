...
- External delivery or contractual impact: `[无 / 外部可见但低风险 / 有合同、合规或客户承诺影响]`
...
- [ ] 有明确 Gate 3 人验收角色。
- [ ] 失败动作能在当前团队内闭环。
- [ ] 外部交付或合同影响存在但不涉及敏感数据、生产关键路径、跨团队接口或难回滚时，默认 Standard，并加强 Gate 4 审查。
...
- [ ] 不可轻易回滚。
- [ ] 跨团队、跨系统或影响外部用户。
- [ ] 外部交付涉及合同、合规、真实客户数据、生产关键路径或难回滚承诺。
- [ ] 涉及重大架构决策或长期标准变更。
