...
Use synthetic-only context. Keep Non-Scope: no API integration, no credential setup, no tenant connectors, no unattended or automated writeback, and no provider comparison.
```
...
| Boundary is preserved | No provider-native validation, no API integration, no credential setup, no tenant connectors, no unattended or automated writeback, no benchmark ranking. |
